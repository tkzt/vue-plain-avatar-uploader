  ...
  <div :style="{ height: actualHeight + 'px' }" class="container">
    ...
    >
      <div
        ...
        :style="{
          ...
          'border-radius': maskShape === 'circle' ? '50%' : maskRectBorderRadius,
          'box-shadow': maskBoxShadow
        }"
        @mousedown="onMouseDown"
      >
        <slot name="action.innerSelectButton" :on="{ click: toSelectImage }" v-if="!image">
    ...
    <div class="actions">
      <div class="action">
        <slot
          name="action.slider"
          :attrs="{ min: minEnlargeTimes, max: maxEnlargeTimes, value: enlargeTimes, disabled: !image }"
      ...
      <div class="action" v-if="hideShapeGroup === undefined">
        ...
        >
          <label class="action-shape">
            <input name="shape" :disabled="!image" type="radio" value="circle" checked v-model="maskShape" style="margin-right: 8px" />圆形
          </label>
          <label class="action-shape">
            <input name="shape" :disabled="!image" type="radio" value="square" v-model="maskShape" style="margin-right: 8px" />方形
          </label>
        </slot>
      </div>

      <div class="action">
        <slot name="action.selectButton" :text="selectorText" :on="{ click: toSelectImage }">
          <button type="button" class="action-button" @click="toSelectImage">{{ selectorText }}</button>
        </slot>
        <slot name="action.confirmButton" :attrs="{ disabled: !image }" :on="{ click: confirm }">
          <button type="button" class="action-button" @click="confirm" :disabled="!image">确定</button>
        </slot>
      </div>
    </div>
  </div>
...
<script>
export default {
  props: ['height', 'maskProps', 'outputProps', 'backgroundColor', 'maxMultiple', 'hideShapeGroup'],
  data() {
    return {
      image: null,
      maskDiameter: 0,
  ...
  computed: {
    ...
    maskBoxShadow() {
      return `${this.maskTransparent ? this.maskBoxShadowColor : '#fff'} 0 0 0 ${this.maskBoxShadowRadius}px`;
    },
    adjusterBgColor() {
    ...
    bgImageSize() {
      return [this.bgImageSrcSize[0] * this.enlargeTimes, this.bgImageSrcSize[1] * this.enlargeTimes];
    },
    distSize() {
    ...
    distBackgroundColor() {
      return (this.outputProps && this.outputProps.backgroundColor) || 'transparent';
    }
  },
  mounted() {
    setTimeout(() => {
  ...
  methods: {
    ...
    onMouseMove(event) {
      ...
      // 更新 bg image 位置
      this.bgImagePostion = [this.bgImagePostion[0] + deltaX, this.bgImagePostion[1] + deltaY];
      this.bgStartPos = [event.x, event.y];
    },
    onMouseUp() {
      if (this.image) {
    ...
    toSelectImage() {
      ...
      input.onchange = e => {
        ...
        targetImage && (this.image = targetImage);
        ...
        img.onload = e => {
          ...
          if (xRatio > yRatio) {
            ...
            this.bgImagePostion = [0, (-height * xRatio + this.containerShape[1]) / 2];
          } else {
            this.bgImageSrcSize = [width * yRatio, height * yRatio];
            this.bgImagePostion = [(-width * yRatio + this.containerShape[0]) / 2, 0];
          }
        };
      };
    },
    initialize() {
      ...
      // mask
      this.maskDiameter = offsetHeight < offsetWidth ? offsetHeight : offsetWidth;
      this.maskBoxShadowRadius = (Math.pow(offsetWidth ** 2 + offsetHeight ** 2, 0.5) - this.maskDiameter) / 2;
    ...
    cropImage(img) {
      ...
      // background
      if (this.distBackgroundColor === 'transparent' && this.distFormat !== 'png') {
        ctx.fillStyle = '#fff';
      } else {
      ...
      ctx.drawImage(
        img,
        (-this.bgImagePostion[0] + (this.containerShape[0] - this.maskDiameter) / 2) * ratio,
        (-this.bgImagePostion[1] + (this.containerShape[1] - this.maskDiameter) / 2) * ratio,
